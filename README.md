...
`startup_delay` | False | By default, initial entity and services check is done immediately after `homeassistant_started` event is fired. Some integrations may require extra time for intitialization so that their entities/services may not yet be ready during watchman check, this is especially true for single-board computers like Raspberry PI. This option allows to postpone automatic startup check for certain amount of seconds. | `0`    

...
async def test_upgrade_wipes_old_db(tmp_path):
    """
    Test Upgrade Scenario (v2 -> v3).
    
    1. Create DB with version 2.
    2. Populate with some data.
    3. Initialize Parser.
    4. Assert DB is wiped (data gone) and version is 3.
    """
...
async def test_downgrade_wipes_future_db(tmp_path):
    """
    Test Downgrade Scenario (v4 -> v3).
    
    1. Create DB with version 4.
    2. Populate with future data.
    3. Initialize Parser.
    4. Assert DB is wiped and version is 3.
    """
    db_path = tmp_path / DB_FILENAME
